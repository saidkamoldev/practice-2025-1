/*** KILO TEXT EDITOR ***/
/*** Авторы: Журакулов Саидкамол, Аширматов Абдукодир ***/
/*** Группа 251-335, Московский Политех, 2025-2026 ***/
/*** Основано на: viewsourcecode.org/snaptoken/kilo  ***/
...
void editorDrawRows(struct abuf *ab) {
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      ...
      /* номер строки — наша модификация */
      char linenum[8];
      int numlen = snprintf(linenum, sizeof(linenum), "%4d ", filerow + 1);
      abAppend(ab, "\x1b[90m", 5); /* серый цвет */
...
void editorRefreshScreen() {
  ...
  snprintf(buf, sizeof(buf), "\x1b[%d;%dH",
    (E.cy - E.rowoff) + 1,
    (E.rx - E.coloff) + 6); /* +6 для отступа номеров строк */
